...
async function main(): Promise<void> {
  ...
  logger.log('get account', await sdk.getAccount(state.accountAddress));
  logger.log('get account members', await sdk.getAccountMembers(state.accountAddress));

  await topUpAccount(state.accountAddress, '0.5');

  logger.log('contract account balance', await getBalance(state.accountAddress));

  logger.log('batch #1', await sdk.batchAddAccountOwner(randomAddress()));
  logger.log('batch #2', await sdk.batchAddAccountOwner(randomAddress()));
