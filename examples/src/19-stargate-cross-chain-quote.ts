...
async function main(): Promise<void> {
  ...
  const privateKey = process.env.FANTOM_PRIVATE_KEY;
  ...
  const FantomUSDC = '0x04068DA6C83AFCFA0e13ba15A6696662335D5B75'; // Fantom - USDC
  ...
  const toChainId: number = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Fantom];
  const fromTokenAddress: string = MaticUSDC;
  const toTokenAddress: string = FantomUSDC;
  ...
  const quoteRequestPayload = {
    ...
    serviceProvider: CrossChainServiceProvider.Stargate, // Optional parameter
  };
  ...
  if (quotes.items.length > 0) {
    ...
    const erc20Contract = sdk.registerContract<ERC20Contract>('erc20Contract', abi, tokenAddress);
